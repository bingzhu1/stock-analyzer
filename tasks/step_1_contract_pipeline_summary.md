...
## 44. Real Validation Execution Glue（Step 3R-3.3C-C-C，one-shot guarded script）

### 44.1 范围

把 Step 3R-3.3C-C-C execution glue design（commit `0bf9151`）+ checkpoint（`90c0b4e`）落到一个 one-shot read-only 装配脚本：

- 串联已 merge 的三个 building block：wrapper（Step 3R-3.3C-B）、real provider（Step 3R-3.3C-C-B）、dry-run orchestrator（Step 3R-3.3A）。
- threshold 锁 `0.60`（v1 seed），cutoff 锁 `"2026-01-01"`；任何其它值 → CLI exit 2。
- 必须显式 `--run-once-real-validation` 才能跑；不传 → exit 2。
- output_dir 必须不存在；orchestrator 拒覆盖。
- DB / market_data.db / `avgo_agent.db.backup_*` 三组 fingerprint before & after，任一变化 → `RuntimeError`。
- 本节只 ship 脚本 + 测试；**真实 W1-W4 validation 仍未运行**。
- 测试以 mock 锁定装配顺序，不读真实 4 csv、不读真实 W4 jsonl、不跑 639 rows、不连真实 DB（fixture sqlite 文件 / fixture backup glob 在受控 cwd）。

### 44.2 改动文件

| 路径 | 类型 | 说明 |
|---|---|---|
| `scripts/run_real_continuous_smoothing_validation_execute.py` | 新增 | one-shot execution glue + CLI |
| `tests/test_run_real_continuous_smoothing_validation_execute.py` | 新增 | 28 focused tests（7 类） |
| `tasks/step_1_contract_pipeline_summary.md` | 修改 | 新增 §44（本节） |
...
| 参数 | 默认 | lock 行为 |
|---|---|---|
| `--run-once-real-validation` | flag，默认 False | 不传 → `refuse_to_run:missing_explicit_opt_in` + exit 2 |
| `--candidate-threshold` | `0.60` | ≠ 0.60 → `refuse_to_run:candidate_threshold_locked:expected=0.6,got=...` + exit 2 |
| `--final-test-cutoff` | `"2026-01-01"` | ≠ `"2026-01-01"` → `refuse_to_run:final_test_cutoff_locked:expected=...,got=...` + exit 2 |
| `--output-dir` | 必填 | 已存在 → `refuse_to_run:output_dir_exists:...` + exit 2 |
...
### 44.5 execution flow（10 步）

```
1. validate_execution_args(args)               # locks
2. db_fp_before = get_db_fingerprint(args.db_path)
3. market_fp_before = get_db_fingerprint("data/market_data.db")  # if exists
4. backup_before = get_backup_count("avgo_agent.db.backup_*")
5. bundle = build_real_validation_inputs(...)          # wrapper (Step 3R-3.3C-B)
6. provider = build_real_regime_label_provider(...)    # real provider (Step 3R-3.3C-C-B)
7. result = run_continuous_smoothing_validation(       # dry-run orchestrator (Step 3R-3.3A)
       bundle["w1_w3_rows"] + bundle["w4_rows"],
...
8. _verify_output_files(args.output_dir)        # 4 文件 exist or RuntimeError
9. assert_db_unchanged(db_fp_before, db_fp_after)
   if market_fp_before: assert_db_unchanged(market_fp_before, market_fp_after)
10. assert_backup_count_unchanged(backup_before, backup_after)
→ build_execution_summary(real_validation_execution_summary.v1)
```

### 44.6 output 文件验证

orchestrator 必须在 `output_dir` 内写完整 4 个文件；缺任一 → `RuntimeError("output_files_missing:[...] in <dir>")`：
...
execution glue **不重写**这些文件；写文件由 orchestrator 在已 merge 的 `_write_outputs_files(...)`（commit `9720e0a` 已在 merge 链）完成。

### 44.7 DB guard

| 监测 | 来源 | 失败行为 |
|---|---|---|
| `avgo_agent.db` mtime_ns + size_bytes | wrapper 提供的 `get_db_fingerprint` | `assert_db_unchanged` raises `RuntimeError("db_modified:...")` |
| `data/market_data.db` mtime_ns + size_bytes（若存在） | 同上（额外 fingerprint） | 同上 |
| `avgo_agent.db.backup_*` glob count | `get_backup_count` | `assert_backup_count_unchanged` raises `RuntimeError("db_backup_count_changed:...")` |

execution glue **不**直接 `import sqlite3`；DB 仅通过 wrapper 接触（wrapper 已用 `mode=ro` URI）。

### 44.8 isolation / forbidden imports

`test_no_forbidden_imports` 锁定（21 项）：

- `yfinance` / `requests` / `urllib` / `urllib3` / `httpx`
- `longbridge` / `broker` / `paper_trade`
- `sqlite3`（execution glue 不直接连 DB）
- `services.prediction_store` / `services.outcome_capture`
- `services.confidence_engine` / `contradiction_engine` / `risk_model` / `soft_metadata_simulator` / `anti_false_exclusion_dashboard` / `regime_diagnostics_dashboard` / `protection_layer_diagnostics` / `historical_replay_training`
- `predict` / `scanner` / `streamlit`
- `ui.protection_layer_diagnostics_renderer`

`test_no_direct_sqlite3_import` 字符串扫：

- `import sqlite3`
- `from sqlite3`

`test_no_hard_required_or_trading_strings` 字符串扫：

- `hard_exclusion_allowed` / `forced_exclusion` / `anti_false_exclusion_triggered`
- `_PROTECTION_LAYER_CONNECTED`
- `simulated_trade` / `no_trade`
- `final_direction` / `final_projection`
- `yfinance` / `paper_trade` / `longbridge`

`test_no_threshold_sweep_or_override_strings` 字符串扫：

- `thresholds = [` / `for threshold in` / `for t in thresholds` / `candidate_thresholds` / `threshold_grid` / `optimize_threshold` / `sweep_threshold`
...
`test_sources_only_locked_threshold_and_cutoff`：模块常量必须以 `LOCKED_CANDIDATE_THRESHOLD = 0.60` + `LOCKED_FINAL_TEST_CUTOFF = "2026-01-01"` 形式存在。

### 44.9 测试覆盖

28 focused tests 全部通过：

| 测试类 | 数量 | 覆盖点 |
|---|---|---|
| `GetBackupCountTests` | 1 | glob cwd backup 计数 / 无关 backup 文件不匹配 |
| `AssertBackupCountUnchangedTests` | 2 | equal pass / mismatch raises |
| `ValidateExecutionArgsTests` | 7 | run_once flag / threshold>lock / threshold<lock / cutoff != "2026-01-01"（3 例） / output_dir 已存在 / 缺必填 path（8 attrs） / happy path 不抛 |
...
| `GuardMismatchTests` | 4 | DB size mismatch / DB mtime mismatch / backup count mismatch / market_data.db mismatch（受控 cwd 模拟） |
...
| `pytest tests/test_run_real_continuous_smoothing_validation.py tests/test_real_regime_label_provider.py tests/test_run_continuous_smoothing_validation.py -q` | **76 passed**（零回归） |
...
- ❌ **没**改 `predict.py` / `run_predict` / `scanner.py` / `prediction_store.py` / `app.py` / DB schema
- ❌ **没**改 04 / 05 / 07 任何 required 字段
- ❌ **没**改 `services/regime_labels_builder.py` / `services/regime_features_builder.py` / `services/regime_validation_helper.py` / `services/continuous_smoothing_candidate.py` / `services/replay_validation_record_adapter.py` / `services/real_regime_label_provider.py` / 任何已有 service / 任何 ui 模块 / 任何 builder
- ❌ **没**改 `scripts/run_continuous_smoothing_validation.py` / `scripts/run_real_continuous_smoothing_validation.py`
- ❌ **没**改任何已有测试文件
- ❌ **没**直接 `import sqlite3`（execution glue 仅通过 wrapper 接触 DB）
- ❌ **没**写 DB；DB / market_data.db / backup count 三组 fingerprint guard
- ❌ **没**接网络（`ast.walk` 锁定 21 项 forbidden import）
- ❌ **没**接 trading API
- ❌ **没**让 `_PROTECTION_LAYER_CONNECTED` 翻 True
- ❌ **没**触碰 2026-01-01 之后 final-test range（CLI lock + wrapper / orchestrator / provider / candidate / adapter / helper / report 6 层 hard stop）
- ❌ **没**真实运行 W1-W4 validation
- ❌ **没**读真实 `data/AVGO.csv` / `NVDA.csv` / `SOXX.csv` / `QQQ.csv`
- ❌ **没**读真实 W4 jsonl
- ❌ **没**跑真实 639 rows
- ❌ **没**扫 / 学 / 反推 candidate_threshold（v1 seed 0.60 锁定，CLI 拒非 0.60）
- ❌ **没**调 SEED coefficients
- ❌ **没** add 任何 W4 / smoke / `logs/regime_validation/` / `logs/prediction_log.jsonl` / DB backup / `agent_loop.py` / `.claude/worktrees/`
