"""scripts/run_continuous_smoothing_validation_v2.py — v2 dry-run orchestrator.

Step 3R-3.3F-C implementation per Step 3R-3.3F-C v2 execution path
design (commit ``18a41d8``) + checkpoint (``fe76252``).

Independent v2 orchestrator. Mirrors the v1 orchestrator
(``scripts.run_continuous_smoothing_validation``) but:

- imports ``services.continuous_smoothing_candidate_v2.build_continuous_smoothing_candidate_v2``
  (NEVER imports the v1 candidate)
- defaults ``candidate_name = "continuous_smoothing_v2"``
- emits ``schema_version = "continuous_smoothing_validation_run_v2.v1"``
...
- never reads DB / network; never imports ``yfinance`` / ``requests`` /
  ...
  ``scanner`` / ``streamlit`` / ``services.continuous_smoothing_candidate``
  (v1) / ``scripts.run_continuous_smoothing_validation`` (v1
  orchestrator) / ``scripts.run_real_continuous_smoothing_validation_execute``
  (v1 glue)
...
- does NOT sweep / learn / pick ``candidate_threshold``; caller passes
  it (default 0.60 is a v2 first-run lock — semantically distinct from
  v1's 0.60; see Step 3R-3.3F-C design §7)
...
risk_score semantics (locked by Step 3R-3.3F design §8):

    risk_score = P̂(prediction will be wrong | features)

The adapter mapping is unchanged from v1 (threshold-on-`risk_score`):

- ``risk_bucket ∈ {high, extreme}`` (i.e., ``risk_score >= threshold``)
  → adapter ``candidate_triggered=True``
- ``risk_bucket ∈ {abstain, low, medium}`` (i.e., ``risk_score < threshold``
  or ``risk_score is None``) → adapter ``candidate_triggered=False``
...
from services.continuous_smoothing_candidate_v2 import (
...
def _enrich_row_with_candidate_v2(
    row: dict[str, Any],
    *,
    ...
    candidate = build_continuous_smoothing_candidate_v2(
        labels,
        as_of_date=analysis_date,
        final_test_cutoff=final_test_cutoff,
    )
    # Use the same `candidate` key as the v1 path so the unchanged
    # adapter (`replay_validation_record_adapter`) reads it.
...
def run_continuous_smoothing_validation_v2(
    ...
    *,
    ...
    """Stitch v2 candidate → adapter → helper for a dry-run validation pass.

    See module docstring for full read-only constraints. The
    adapter / helper / wrapper / real provider are reused unchanged
    (those are protocol-layer modules, 3R-4 locked).
    """
    ...
    for row in replay_rows:
        ...
        if analysis_date >= final_test_cutoff:
            ...
            continue
        enriched, row_warnings, refusal_flag = _enrich_row_with_candidate_v2(
            row,
    ...
    run_manifest = _build_run_manifest_v2(
        candidate_name=candidate_name,
        candidate_threshold=candidate_threshold,
        w4_manifest_status=w4_manifest_status,
        final_test_cutoff=final_test_cutoff,
    ...
    summary_md = _render_summary_md_v2(
        run_manifest=run_manifest, report=report
    )

    if write_outputs:
        out_path = Path(output_dir)  # type: ignore[arg-type]
        _write_outputs_files_v2(
            output_dir=out_path,
...
if __name__ == "__main__":
    print(
        "This script exposes run_continuous_smoothing_validation_v2 as a "
        "library function.\n"
        "Direct CLI execution is intentionally not provided here; integration "
        "is via the v2 execution glue.",
        flush=True,
    )
    raise SystemExit(0)
