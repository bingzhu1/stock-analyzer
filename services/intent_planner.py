...
def plan_intent(text: str) -> dict[str, Any]:
    ...
    # Absolute date range — takes priority over relative lookback for query steps.
    _parsed_range = parse_date_range(raw_text)
    date_inverted_msg: str | None = None
    if _parsed_range and _parsed_range[0] > _parsed_range[1]:
        date_inverted_msg = (
            f"起始日期晚于结束日期（{_parsed_range[0]} > {_parsed_range[1]}），请检查输入。"
        )
        _parsed_range = None
    date_range = _parsed_range
    partial_date = has_partial_date_signals(raw_text) and date_range is None and date_inverted_msg is None
    ...
    if date_inverted_msg:
        warnings.append(date_inverted_msg)
        blocking = True
    ...
    # ── 2. query ──────────────────────────────────────────────────────────────
        ...
        if not symbols:
            ...
            blocking = True
        if partial_date:
            # User intended a date range but it could not be fully parsed — fail
            # loudly instead of silently falling back to the default 20-day window.
            warnings.append(
                "检测到日期范围表达，但未能完整解析起止日期。"
                "请补充完整的起止日期（例如：2026年1月15日至2月9日）。"
            )
            blocking = True
        if date_range:
