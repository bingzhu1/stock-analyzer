...
def parse_with_ai_primary(
    text: str,
    *,
    _today: str | None = None,
) -> tuple[ParsedTask, dict[str, Any], bool]:
    """AI-first parse. Falls back to rule-based if AI is unavailable or fails.

    Priority:
      1. ai_task_parser (rich schema, year inference, full intent coverage)
      2. parse_with_ai_fallback (existing rule-based + light AI fallback)
    ...
    tuple of (parsed, plan, ai_used)
        ai_used is True when the primary AI parser produced the result.
    """
    import os

    if os.getenv("OPENAI_API_KEY", "").strip():
        try:
            from services.ai_task_parser import parse_task
            from services.plan_normalizer import normalize

            ai_schema = parse_task(text, _today=_today)
            if ai_schema is not None:
                parsed, plan = normalize(text, ai_schema)
                return parsed, plan, True
        except Exception as exc:
            import warnings as _w
            _w.warn(f"[parse_with_ai_primary] primary parser failed, falling back: {exc}")
