...
_MISSING_RS_VALUES = {"", "none", "unavailable", "unknown"}


def _rs_value_missing(v: object) -> bool:
    return v is None or str(v).lower() in _MISSING_RS_VALUES
...
def _external_confirmation_missing(scan_result: dict[str, Any]) -> bool:
    ...
    return bool(values) and all(_rs_value_missing(v) for v in values)
