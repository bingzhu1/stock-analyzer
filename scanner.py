...
def build_recent_avgo_window(
    coded_df: pd.DataFrame,
    target_date_str: str,
    window: int = 20,
) -> list[dict[str, Any]]:
    """Return the latest AVGO rows through target date for primary projection."""
    if coded_df.empty:
        return []
    ...
    df["Date"] = pd.to_datetime(df["Date"])
    target_ts = pd.to_datetime(target_date_str)
    df = df[df["Date"] <= target_ts].sort_values("Date").tail(window)
    ...
    for raw in df[available_fields].to_dict("records"):
        row: dict[str, Any] = {}
...
def run_scan(
    ...
    return {
        ...
        "avgo_recent_20": build_recent_avgo_window(coded_df, target_date_str, window=20),
